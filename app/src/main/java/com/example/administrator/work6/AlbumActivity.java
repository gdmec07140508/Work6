...
public class AlbumActivity extends AppCompatActivity {
    ...
    public String[] loadAlbum(){
        String patName = android.os.Environment.getExternalStorageDirectory().getPath()+"/com.demo.pr4";
        File file=new File(patName);
        Vector<Bitmap>fileName=new Vector<>();
        if (file.exists()&&file.isDirectory()){
            String[] str=file.list();
            for(String s:str){
                if (new File(patName+"/"+s).isFile()){
                    fileName.addElement(loadImage(patName+"/"+s));
                }
            }
